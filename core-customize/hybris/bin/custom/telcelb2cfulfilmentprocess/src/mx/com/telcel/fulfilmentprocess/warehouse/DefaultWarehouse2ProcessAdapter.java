 ...
 */
package mx.com.telcel.fulfilmentprocess.warehouse;
        ...
        for (final ConsignmentProcessModel process : consignment.getConsignmentProcesses()) {
            ...
            if (orderModel instanceof TelcelReplacementOrderModel) {
                getBusinessProcessService().triggerEvent(
                        process.getCode() + "_" + Telcelb2cFulfilmentProcessConstants.REPLACEMENT_WAIT_FOR_WAREHOUSE);
            } else if (CollectionUtils.isEmpty(orderModel.getDeviceComboList())) {
                getBusinessProcessService().triggerEvent(
                        process.getCode() + "_" + Telcelb2cFulfilmentProcessConstants.WAIT_FOR_WAREHOUSE);
            } else {
                getBusinessProcessService().triggerEvent(
                        process.getCode() + "_" + Telcelb2cFulfilmentProcessConstants.WAIT_FOR_TELMEX_WAREHOUSE);
            }
        }
    }
