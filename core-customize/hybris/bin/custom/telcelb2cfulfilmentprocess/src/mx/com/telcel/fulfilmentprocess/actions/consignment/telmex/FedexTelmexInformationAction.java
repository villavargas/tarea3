...
/**
 * The type Fedex telmex information action.
 */
public class FedexTelmexInformationAction extends AbstractAction<ConsignmentProcessModel>
{

	private static final Logger LOG = Logger.getLogger(FedexTelmexInformationAction.class);
	...
	public String execute(final ConsignmentProcessModel consignmentProcessModel) throws Exception
	{
		...
		//final FedexResponse fedexResponse = (FedexResponse) fedexService.createShipment(request, headers);
	...
	private String setResponseValues(ConsignmentProcessModel consignmentProcessModel, FedexRequest request, HeaderRequest headers, ConsignmentModel consignmentModel, Object response) throws KeyManagementException, NoSuchAlgorithmException, KeyStoreException, IOException {
		...
		notifyProductOrderTelmex.notifyProductOrderStateChange(consignmentModel, consignmentModel.getFedexGuide());
	...
	{
		LOG.info("#######################################- FEDEX Guide: " + fedexGuide);
										  ...
										  final ConsignmentModel consignment, final boolean flag)
	{
		...
		for (final ConsignmentEntryModel entryModel : consignment.getConsignmentEntries())
		{
			...
			c1OICh.setValue(abstractOrderEntryModel.getSeriesIMEI().get(0).getClaveSAT());
			final Characteristic c2OICh = new Characteristic();
			c2OICh.setName(Config.getParameter(ORDER_FEDEX_DESCRIPTION));
			c2OICh.setValue(abstractOrderEntryModel.getSeriesIMEI().get(0).getDescriptionSAT());
			...
			totalNoOfGoods ++;
			/*if(abstractOrderEntryModel.getSeriesICCID() != null) {
				...
				c1OICh2.setValue(abstractOrderEntryModel.getSeriesICCID().get(0).getClaveSAT());
				final Characteristic c2OICh2 = new Characteristic();
				c2OICh2.setName(Config.getParameter(ORDER_FEDEX_DESCRIPTION));
				c2OICh2.setValue(abstractOrderEntryModel.getSeriesICCID().get(0).getDescriptionSAT());
				final Characteristic c3OICh2 = new Characteristic();
				c3OICh2.setName("merchandiseValue");
				for(DeviceComboComponentModel deviceComboModel : consignment.getOrder().getDeviceComboList().get(0).getComponentList()){
					if(abstractOrderEntryModel.getSeriesICCID().get(0).getSku().equals(deviceComboModel.getOrderItemPartNumber()) ){
						c3OICh2.setValue(String.valueOf(deviceComboModel.getOrderItemPriceWithoutTax()));
						precioCompleto += Double.valueOf(deviceComboModel.getOrderItemPriceWithoutTax());
					}
				}
				final Characteristic c4OICh2 = new Characteristic();
				c4OICh2.setName("merchandiseTotal");
				c4OICh2.setValue(String.valueOf(abstractOrderEntryModel.getQuantity()));
		...
		String firstName = StringUtils.isNotEmpty(consignment.getOrder().getDeliveryAddress().getFirstname()) ? consignment.getOrder().getDeliveryAddress().getFirstname() : StringUtils.EMPTY;
		String lastName = StringUtils.isNotEmpty(consignment.getOrder().getDeliveryAddress().getLastname()) ? consignment.getOrder().getDeliveryAddress().getLastname() : StringUtils.EMPTY;
		String middlename = StringUtils.isNotEmpty(consignment.getOrder().getDeliveryAddress().getMiddlename()) ? consignment.getOrder().getDeliveryAddress().getMiddlename() : StringUtils.EMPTY;
		individual2.setName(firstName + " " + lastName);
		individual2.setLastName(lastName);
		individual2.setSecondLastName(middlename);
		...
		cp2.setValue(StringUtils.isNotEmpty(consignment.getOrder().getDeliveryAddress().getStreetname()) ? consignment.getOrder().getDeliveryAddress().getStreetname() : StringUtils.EMPTY);
		...
		String district = StringUtils.isNotEmpty(consignment.getOrder().getDeliveryAddress().getDistrict()) ? consignment.getOrder().getDeliveryAddress().getDistrict() : StringUtils.EMPTY;
		address2.setAddressLine1(numExt+" "+numInt + district);
