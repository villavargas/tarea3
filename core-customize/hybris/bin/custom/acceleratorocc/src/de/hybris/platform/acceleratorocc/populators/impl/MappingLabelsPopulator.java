 ...
 */
package de.hybris.platform.acceleratorocc.populators.impl;
...
public class MappingLabelsPopulator extends AbstractRequestPopulator<CreateSubscriptionRequest, PaymentData>
{
	...
	public static final String HYBRIS_BILLTO_REGION = "hybris_billTo_region";
	...
	public static final String HYBRIS_BILLTO_PHONE_NUMBER = "hybris_billTo_phone_number";
	...
	public void populate(final CreateSubscriptionRequest source, final PaymentData target)
	{
		...
		final String paymentProvider = getPaymentProvider();
	...
	protected String getPaymentProvider()
	{
		if (getCartService().hasSessionCart())
		{
			...
			final String paymentProvider = sessionCart.getStore().getPaymentProvider();
			if (StringUtils.isNotBlank(paymentProvider))
			{
				return paymentProvider.toLowerCase(Locale.ENGLISH);
			}
			else
			{
				throw new IllegalStateException("There is no payment provider.");
			}
		}
		else
		{
			throw new IllegalStateException("There is no session cart. No possibility to retrieve payment provider.");
		}
	}
	...
	private void populateAddressLabels(final PaymentData target, final String paymentProvider, final Configuration config)
	{
		...
		target.getMappingLabels().put(HYBRIS_BILLTO_REGION, config.getString(paymentProvider + PAYMENT_LABEL_BILLTO_REGION));
		...
		target.getMappingLabels()
				.put(HYBRIS_BILLTO_PHONE_NUMBER, config.getString(paymentProvider + PAYMENT_LABEL_BILLTO_PHONE_NUMBER));
	}
