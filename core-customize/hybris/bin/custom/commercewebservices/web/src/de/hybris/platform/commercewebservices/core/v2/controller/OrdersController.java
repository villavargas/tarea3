...
import de.hybris.platform.commercewebservices.core.v2.helper.OrdersHelper;
...
public class OrdersController extends BaseCommerceController
{
	...
	private static final String STATUS_ORDER_CONFIRMATION = "order.history.status.order.confirmation";
			...
			final String fields, final HttpServletResponse response)
	{
		...
		updateOrderHistoryValues(orderHistoryList);
	...
	final String fields) throws PaymentAuthorizationException, InvalidCartException, NoCheckoutCartException
	{
		...
		//authorized
		if (!telcelCheckoutFacade.authorizePaymentCustom(null))
		{
	...
	private void updateOrderHistoryValues(final OrderHistoryListWsDTO orderHistoryList)
	{
		for (final OrderHistoryWsDTO oh : orderHistoryList.getOrders())
		{
			try
			{
				final OrderModel order = getOrderModel(oh.getCode());
				if (Objects.nonNull(order))
				{
					//all orders with status VALID_VOUCHER, are updated on site to Order Confirmed
					if(order.getStatus().equals(OrderStatus.VALID_VOUCHERS)) {
						oh.setStatusDisplay(getLocalizedString(STATUS_ORDER_CONFIRMATION));
					}else{
						oh.setStatusDisplay(enumerationService.getEnumerationName(order.getStatus(), new Locale("es", "MX")));
					}
					oh.setPaymentStatus(order.getPaymentStatus().getCode());
					oh.setPaymentStatusDescription(
	...
	private OrderModel getOrderModel(final String orderCode)
	{
		...
		{
			final OrderModel order = customerAccountService.getOrderForCode(orderCode, baseStoreModel);
			return order;
		}
		return null;
	}
