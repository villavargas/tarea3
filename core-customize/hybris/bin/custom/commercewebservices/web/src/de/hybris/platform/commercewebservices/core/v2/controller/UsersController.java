...
public class UsersController extends BaseCommerceController
{
	...
	public static final String USER_MAPPER_CONFIG = "firstName,lastName,titleCode,currency(isocode),language(isocode),birthdate,phoneNumber,curp";
	...
	public UserWsDTO getUser(@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
			...
			CertificateException, ParseException {
		LOG.info("Ejecuta Lookup!!");
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		LOG.info(customerData.getUid());
		String userID = customerData.getUid();
		...
		if(lookupUser.getUser().getEmailone() != null){
			customerData.setDetailEmail(lookupUser.getUser().getEmailone());
		} else {
			customerData.setDetailEmail("");
		}
		if(lookupUser.getUser().getPhonenumberone() != null){
			customerData.setPhoneNumber(removePrefix(lookupUser.getUser().getPhonenumberone(), "52"));
		} else {
			customerData.setPhoneNumber("");
		}
		if(lookupUser.getUser().getUsergivenname() != null){
			if(lookupUser.getUser().getUserfamilyname() != null){
				customerData.setFirstName(lookupUser.getUser().getUsergivenname());
				customerData.setLastName(lookupUser.getUser().getUserfamilyname());
			} else {
				ArrayList<String> completeName = separateSurnames(lookupUser.getUser().getUsergivenname());
				if (completeName.size() > 2) {
					if (completeName.size() > 3) {
						if (completeName.size() > 4) {
							customerData.setFirstName(completeName.get(0).trim() + " " + completeName.get(1).trim()+ " " + completeName.get(2).trim());
							customerData.setLastName(completeName.get(3).trim() + " " +completeName.get(4).trim());
						} else {
							customerData.setFirstName(completeName.get(0).trim() + " " + completeName.get(1).trim());
							customerData.setLastName(completeName.get(2).trim() + " " +completeName.get(3).trim());
						}
					} else {
						customerData.setFirstName(completeName.get(0).trim());
						customerData.setLastName(completeName.get(1).trim() + " " +completeName.get(2).trim());
					}
				} else {
					if(completeName.size() == 1){
						customerData.setFirstName(completeName.get(0).trim());
					} else {
						customerData.setFirstName(completeName.get(0).trim());
						customerData.setLastName(completeName.get(1).trim());
					}
				}
			}
		} else {
			LOG.info("esta vacio el nombre: "+ lookupUser.getUser().getUsergivenname());
			...
			if (completeName.size() > 2) {
				if (completeName.size() > 3) {
					if (completeName.size() > 4) {
						customerData.setFirstName(completeName.get(0).trim() + " " + completeName.get(1).trim()+ " " + completeName.get(2).trim());
						customerData.setLastName(completeName.get(3).trim() + " " +completeName.get(4).trim());
						//customerData.setLastNameSecond(completeName.get(4).trim());
					} else {
						customerData.setFirstName(completeName.get(0).trim() + " " + completeName.get(1).trim());
						customerData.setLastName(completeName.get(2).trim() + " " +completeName.get(3).trim());
						//customerData.setLastNameSecond(completeName.get(3).trim());
					}
				} else {
					customerData.setFirstName(completeName.get(0).trim());
					customerData.setLastName(completeName.get(1).trim() + " " +completeName.get(2).trim());
					//customerData.setLastNameSecond(completeName.get(2).trim());
				}
			} else {
				if(completeName.size() == 1){
					customerData.setFirstName(completeName.get(0).trim());
					if(lookupUser.getUser().getUserfamilyname() != null){
						ArrayList<String> surnames = separateSurnames(lookupUser.getUser().getUserfamilyname());
						if(surnames.size() > 1){
							customerData.setLastName(surnames.get(0).trim() + " " +surnames.get(1).trim());
							//customerData.setLastNameSecond(surnames.get(1).trim());
						} else {
							customerData.setLastName(surnames.get(0).trim());
						}
					} else {
						customerData.setLastName("");

					}//se cierra condición de userfanulyname
				} else {
					customerData.setFirstName(completeName.get(0).trim());
					customerData.setLastName(completeName.get(1).trim());
				}
		...
		if(lookupUser.getUser().getBirthdate() != null){
			if(lookupUser.getUser().getBirthdate().contains("-")){
				SimpleDateFormat formato = new SimpleDateFormat("yyyy-MM-dd");
				Date birtdate = formato.parse(lookupUser.getUser().getBirthdate());
				customerData.setBirthdate(birtdate);
			} else {
				SimpleDateFormat formato = new SimpleDateFormat("yyyy/MM/dd");
				Date birtdate = formato.parse(lookupUser.getUser().getBirthdate());
				customerData.setBirthdate(birtdate);
			...
			customerData.setBirthdate(null);
		}
		if (lookupUser.getUser().getEmailverifiedone() != null){
			customerData.setEmailverified(lookupUser.getUser().getEmailverifiedone());
		} else {
			customerData.setEmailverified(false);
		}

		return getDataMapper().map(customerData, UserWsDTO.class);
	}
	...
	public static ArrayList<String> separateSurnames(String apellidos) {
		String[] articulos = new String[]{"de", "del", "la", "los", "las", "De", "Del", "La", "Los", "Las", "o", "O", "Mac", "mac", "di", "Di", "da", "do", "dos", "san", "d"};
		ArrayList<String> apellidosArrayList = new ArrayList<String>();
		String[] nameSurnameTemp = apellidos.split(" ");
		String aux = "";
		for (String i : nameSurnameTemp) {
			if (Arrays.stream(articulos).anyMatch(i::equals)) {
				aux += " "+i;
			} else {
				apellidosArrayList.add(aux + " "+i);
				aux = "";
			}
		}
		return apellidosArrayList;
	}
