 ...
 */
public abstract class AbstractAsyncConverter<SOURCE, TARGET>
        implements Converter<SOURCE, TARGET>, Populator<SOURCE, TARGET>, InitializingBean, BeanNameAware
{
    private static final Logger LOG = Logger.getLogger(AbstractAsyncConverter.class);
    private Class<TARGET> targetClass;
    final private int asyncTries = 60;
    ...
    {
        asyncpopulate(source, prototype);
        return prototype;
    }

    private void asyncpopulate(final SOURCE source, final TARGET target)
    {
        int v1=0;
        while(true)
        {
            ...
            }catch (JaloObjectNoLongerValidException ex)
            {
                ...
                LOG.warn("possible async persistence issue, trying again "+ex.getMessage());
                try {
                    Thread.sleep(100);
    ...
    public void afterPropertiesSet() throws Exception
    {
        if (targetClass == null)
        {
            final Class<? extends AbstractAsyncConverter> cl = this.getClass();
            final Method createTargetMethod = ReflectionUtils.findMethod(cl, "createTarget");
            if (de.hybris.platform.converters.impl.AbstractConverter.class.equals(createTargetMethod.getDeclaringClass()))
            {
