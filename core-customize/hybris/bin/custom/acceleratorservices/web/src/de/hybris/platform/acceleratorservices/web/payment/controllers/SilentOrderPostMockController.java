...
public class SilentOrderPostMockController
{
			...
			final HttpServletRequest request, final Model model)
	{
		...
		{
			LOG.debug("SilentOrderPostMockController/process bindingResult.hasErrors()");
		...
		else
		{
			...
			LOG.debug("SilentOrderPostMockController/process bindingResult success");
			...
			if(form.getCard_accountNumber() != null){
		...
		{
			LOG.debug("true getSopMockUrlValidator().validatePaymentResponseUrl(postUrl)");

			model.addAttribute("postUrl", postUrl);
		}
		else
		{
			LOG.debug("false !getSopMockUrlValidator().validatePaymentResponseUrl(postUrl)");
		...
		{
			LOG.debug("getSopMockUrlValidator().validateMerchantCallBackUrl(merchantCallbackUrl)");

			sendMerchantCallback(merchantCallbackUrl, params);
		}
		LOG.debug("SOP_REDIRECT_POST_PAGE = "+SOP_REDIRECT_POST_PAGE);
	...
	{
		LOG.debug("sendMerchantCallback");
		HttpURLConnection conn = null;
		try
		{
			final URL url = new URL(merchantCallbackUrl);
			LOG.debug("sendMerchantCallback url = "+url.toString());

			conn = (HttpURLConnection) url.openConnection();
			if (conn instanceof HttpsURLConnection)
			{
				LOG.debug("conn instanceof HttpsURLConnection");
