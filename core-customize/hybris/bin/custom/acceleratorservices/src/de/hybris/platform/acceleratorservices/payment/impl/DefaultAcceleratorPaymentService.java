...
import de.hybris.platform.acceleratorservices.payment.strategies.*;
...
public class DefaultAcceleratorPaymentService implements PaymentService
{
	...
	private ReferencePaymentInfoCreateStrategy referencePaymentInfoCreateStrategy;
	private SpeiPaymentInfoCreateStrategy speiPaymentInfoCreateStrategy;
...
//		}
		...
		if (DecisionsEnum.ACCEPT.name().equalsIgnoreCase(response.getDecision()))
		{
			...
			if (getSignatureValidationStrategy().validateSignature(response.getSubscriptionInfoData()))
			{
				...
				if("CARD".equalsIgnoreCase(response.getPaymentInfoData().getPaymentType())){
					final CreditCardPaymentInfoModel cardPaymentInfoModel = getCreditCardPaymentInfoCreateStrategy().saveSubscription(
							customerModel, response.getCustomerInfoData(), response.getSubscriptionInfoData(),
							response.getPaymentInfoData(), saveInAccount);
					paymentSubscriptionResult.setStoredCard(cardPaymentInfoModel);
				...
				}else if("REFERENCE".equalsIgnoreCase(response.getPaymentInfoData().getPaymentType())){
					final ReferencePaymentInfoModel referencePaymentInfoModel = getReferencePaymentInfoCreateStrategy().saveSubscription(
							customerModel, response.getCustomerInfoData(), response.getSubscriptionInfoData(),
							response.getPaymentInfoData(), saveInAccount);

					paymentSubscriptionResult.setStoredReference(referencePaymentInfoModel);

					// Check if the subscription has already been validated
					final CCPaySubValidationModel subscriptionValidation = getCreditCardPaymentSubscriptionDao()
							.findSubscriptionValidationBySubscription(referencePaymentInfoModel.getSubscriptionId());
					if (subscriptionValidation != null)
					{
						referencePaymentInfoModel.setSubscriptionValidated(true);
						getModelService().save(referencePaymentInfoModel);
						getModelService().remove(subscriptionValidation);
					}
				}else if("SPEI".equalsIgnoreCase(response.getPaymentInfoData().getPaymentType())){
					final SpeiPaymentInfoModel speiPaymentInfoModel = getSpeiPaymentInfoCreateStrategy().saveSubscription(
							customerModel, response.getCustomerInfoData(), response.getSubscriptionInfoData(),
							response.getPaymentInfoData(), saveInAccount);

					paymentSubscriptionResult.setStoredSpei(speiPaymentInfoModel);

					// Check if the subscription has already been validated
					final CCPaySubValidationModel subscriptionValidation = getCreditCardPaymentSubscriptionDao()
							.findSubscriptionValidationBySubscription(speiPaymentInfoModel.getSubscriptionId());
					if (subscriptionValidation != null)
					{
						speiPaymentInfoModel.setSubscriptionValidated(true);
						getModelService().save(speiPaymentInfoModel);
						getModelService().remove(subscriptionValidation);
					}
				}
	...
	public ReferencePaymentInfoCreateStrategy getReferencePaymentInfoCreateStrategy() {
		return referencePaymentInfoCreateStrategy;
	}

	public void setReferencePaymentInfoCreateStrategy(ReferencePaymentInfoCreateStrategy referencePaymentInfoCreateStrategy) {
		this.referencePaymentInfoCreateStrategy = referencePaymentInfoCreateStrategy;
	}

	public SpeiPaymentInfoCreateStrategy getSpeiPaymentInfoCreateStrategy() {
		return speiPaymentInfoCreateStrategy;
	}

	public void setSpeiPaymentInfoCreateStrategy(SpeiPaymentInfoCreateStrategy speiPaymentInfoCreateStrategy) {
		this.speiPaymentInfoCreateStrategy = speiPaymentInfoCreateStrategy;
	}
}
